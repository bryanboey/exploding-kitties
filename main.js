...
class Game {
	...
	start(e) {
		...
		player1 = {
			name: "Player 1",
			hand: [new Defuse()],
			column: document.querySelector("#p1"),
		};
		player2 = {
			name: "Player 2",
			hand: [new Defuse()],
			column: document.querySelector("#p2"),
		};
	...
	drawCard() {
		...
		if (game.extraTurn === true) {
			...
			let drawnCard =
				currentPlayer.hand[currentPlayer.hand.length - 1].getHTML();
	...
	switch (x) {
		...
		case "Nope":
			if (discardedCardsArray[0] === "Attack") {
				...
				discardedCardsArray[0] === "Skip" &&
				discardedCardsArray[1] === "Attack"
			) {
		...
		case "Favor":
			const cardToSteal =
				nextPlayer.hand[
					Math.floor(Math.random() * nextPlayer.hand.length)
				];
			currentPlayer.hand.push(
				nextPlayer.hand.splice(
					nextPlayer.hand.indexOf(cardToSteal),
					1
				)[0]
			); // stealing card
...
function appendDiscardedCards(object) {
	...
	currentPlayer.hand.splice(
		currentPlayer.hand.findIndex((x) => x.name === object.id),
		1
	);
}
...
function checkDrawnCard(drawnCard, player, columnToAppend) {
	if (
		drawnCard.id === "ExplodingKitten" &&
		player.hand.findIndex((x) => x.name === "Defuse") !== -1
	) {
		player.hand.splice(
			player.hand.findIndex((x) => x.name === "Defuse"),
			1
		); // remove from array
		player.hand.splice(
			player.hand.findIndex((x) => x.name === "ExplodingKitten"),
			1
		); // remove from array
